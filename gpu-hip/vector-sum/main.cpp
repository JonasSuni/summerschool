...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        hipMemcpyAsync((dB[i]),&(hB[dec[i].start]),dec[i].len*sizeof(double),hipMemcpyHostToDevice,strm[i]);
        hipLaunchKernelGGL(vector_add,dim3(10),dim3(ThreadsInBlock),0,strm[i],dC[i],dA[i],dB[i],dec[i].len);
        hipMemcpyAsync(&(hC[dec[i].start]),(dC[i]),dec[i].len*sizeof(double),hipMemcpyDeviceToHost);
    }
