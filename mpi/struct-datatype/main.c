...
int main(int argc, char *argv[])
{
  ...
  if (myid == 0) {
    for (i=0; i < reps; i++) // multiple sends for better timing
      ...
      MPI_Send( &particles , psize*n , MPI_BYTE , 1 , 1 , MPI_COMM_WORLD);

  } else if (myid == 1) {
    for (i=0; i < reps; i++)
      // TODO: receive
      // MPI_Recv( &particles , n , particletype , 0 , 1 , MPI_COMM_WORLD , MPI_STATUS_IGNORE);
      MPI_Recv( &particles , psize*n , MPI_BYTE , 0 , 1 , MPI_COMM_WORLD , MPI_STATUS_IGNORE);
  }
